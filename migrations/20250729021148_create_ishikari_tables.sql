...
CREATE TYPE ishikari_job_state AS ENUM (
    'available',
    'scheduled',
    'executing',
    'retryable',
...
CREATE TABLE ishikari_jobs (
    id bigserial PRIMARY KEY,
    state ishikari_job_state NOT NULL DEFAULT 'available'::ishikari_job_state,
...
COMMENT ON TABLE ishikari_jobs IS '1';

-- Create performance indexes
CREATE INDEX ishikari_jobs_state_queue_priority_scheduled_at_id_index 
ON ishikari_jobs(state, queue, priority, scheduled_at, id);

CREATE INDEX ishikari_jobs_args_index 
ON ishikari_jobs USING gin (args);

CREATE INDEX ishikari_jobs_meta_index 
ON ishikari_jobs USING gin (meta);

-- Create notification function
CREATE OR REPLACE FUNCTION ishikari_jobs_notify()
RETURNS trigger AS $$
DECLARE
    channel text;
    notice json;
...
CREATE TRIGGER ishikari_jobs_notify_trigger
    AFTER INSERT ON ishikari_jobs
    FOR EACH ROW
    EXECUTE FUNCTION ishikari_jobs_notify();